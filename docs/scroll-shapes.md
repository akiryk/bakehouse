...
- They sit **between the midground mat and the foreground stage** in the z-stack
  (`--z-shapes: 20` — above the mat at z-10, below the foreground stage at z-30 and the nav
  at z-35). The nav is persistent UI, not atmosphere, so it always renders above the shapes
  and carries its own opaque background (`--color-nav-background`) so shapes never visually
  wash it out — see `docs/navigation.md`.
